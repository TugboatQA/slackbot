/**
 * @file
 *
 * Handle giving/taking karma.
 *
 * Karma is stored team side, so any channel that the bot is in for this team
 * will use the same set of karma data.
 *
 *  Examples:
 *      @eojthebrave++
 *      cats++
 *      tacos--
 *      :taylor:++
 */

const util = require('util');

const controller = require('../controller');

// Give/take karma.
//
// Examples:
// - @eojthebrave++
// - cats++
// - tacos--
// - :taylor:++
controller.hears(/([^-]+)[-+]{2,}$/, ['ambient', 'direct_message', 'direct_mention', 'mention'], changeKarma);

// Retrieve existing karma for a term
//
// Matches requests like:
// - @bot karma tacos?
// - @bot karma tacos
controller.hears(/^karma\s*@?(.+)/, ['direct_mention', 'direct_message'], getKarma);

/**
 * Get existing karma
 *
 * @param bot
 * @param message
 */
async function getKarma(bot, message) {
    let karma, result, text;
    let index = message.match[1].trim();

    // Strip off the trailing ? if there is one. Sometimes people ask questions
    // with a question mark.
    if (index.slice(-1) === '?') {
        index = index.slice(0, -1);
    }

    // Check if the requested karma is for a user
    const user = await isUser(index, bot);
    if (user) {
        index = user.id;
        text = user.real_name;
    } else {
        index = index.toLowerCase();
        text = index;
    }

    // Load the team data
    try {
        karma = await util.promisify(controller.storage.teams.get)(`${message.team}_karma`);
        result = karma[index] || 0;
    } catch (err) {
        result = 0;
    }

    bot.reply(message, `${text} has karma ${result}`);
}

/**
 * Increment or decrement karma
 *
 * @param bot
 * @param message
 */
async function changeKarma(bot, message) {
    let index, text, karma;

    // Clean up the message text a bit
    text = message.match[1].trim();

    // Ignore any matches greater than 34 chars, this is arbitrary, existing
    // DB has entries up to 34 chars in length, so 34 seems reasonable
    if (text.length > 34) {
        return;
    }

    const user = await isUser(text, bot);
    if (user) {
        // If we found a slack user that matches then we can store karma on
        // their user ID, and display their preferred real name
        index = user.id;
        text = user.real_name;
    } else {
        index = text.toLowerCase();
    }

    if (await isNarcissism(message.user, index, bot)) {
        bot.reply(message, `Nice try <@${user.id}>, but no...`);
        return;
    }

    // Load the team karma data
    try {
        karma = await util.promisify(controller.storage.teams.get)(`${message.team}_karma`);
    } catch (err) {
        karma = {};
    }

    // Initialize karma that didn't exist before
    karma[index] = karma[index] || 0;

    // Increment karma
    if (/.*\+\+/.test(message.text)) {
        karma[index] += 1;
        bot.api.reactions.add({ timestamp: message.ts, channel: message.channel, name: 'tada' }, err => {
            if (err) {
                bot.botkit.log('Failed to add emoji reaction', err);
            }
        });
    }

    // Decrement karma
    if (/.*--/.test(message.text)) {
        karma[index] -= 1;
        bot.api.reactions.add({ timestamp: message.ts, channel: message.channel, name: 'panda_face' }, err => {
            if (err) {
                bot.botkit.log('Failed to add emoji reaction', err);
            }
        });
    }

    try {
        karma.id = `${message.team}_karma`;
        await util.promisify(controller.storage.teams.save)(karma);
        bot.reply(message, `${text} has karma of ${karma[index]}`);
    } catch (err) {
        bot.botkit.log(`Failed to update karma for ${text}`);
    }
}

/**
 * Check if a string of text is actually the ID or name of a Slack user.
 * If the string is a Slack user ID, we convert it to their username.
 *
 * @param text
 * @param bot
 * @returns {*}
 */
async function isUser(text, bot) {
    try {
        // Try looking for a slack-formated user ID
        const matches = /^<@([-._a-z0-9]+)>/i.exec(text);
        const info = await util.promisify(bot.api.users.info)({ user: matches[1] });
        return info.user;
    } catch (err) {
        // User not found, carry on
    }

    try {
        // Search for a user by username or profile display name
        const matches = /^@?(.*)/i.exec(text);
        const users = await util.promisify(bot.api.users.list)({ limit: 1000 });
        return users.members.filter(user => user.name === matches[1] || user.profile.display_name === matches[1])[0];
    } catch (err) {
        // User not found, carry on
    }

    return false;
}

/**
 * Make sure a user is not trying to change their own karma
 *
 * @param giver    - The user giving the karma
 * @param receiver - The user receiving the karma
 * @param bot
 */
async function isNarcissism(giver, receiver, bot) {
    const info = await util.promisify(bot.api.users.info)({ user: giver });
    return receiver === info.user.id;
}
