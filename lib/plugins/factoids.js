...
async function getFact(bot, message, key) {
    ...
    const user = await slackbot.getUser(index, bot);

    if (user) {
        fact = factoids[user.id];
        fact.key = user.real_name;
    } else {
        fact = factoids[index] || null;
    }
...
async function setFact(bot, message) {
    ...
    if (!existing) {
        ...
        bot.startConversation(message, (err, convo) => {
            convo.say(`I already have a factoid for "${fact.key}". It says "${factString(existing)}`);
...
async function forgetFact(bot, message) {
    const factoids = await loadFacts(message.team);
    const key = message.match[1].trim().toLowerCase();
    ...
    const responses = [
        {
            ...
            callback: async (response, convo) => {
                try {
                    ...
                    convo.say(`Okay, I have forgotten about "${key}"`);
                } catch (err) {
                    convo.say(`There was a problem forgetting about "${key}". ${err}`);
                }
                convo.next();
            },
        },
    ...
    bot.startConversation(message, (err, convo) => {
        convo.addQuestion(`Are you sure you want me to forget "${key}"? Say YES, or NO`, responses, {}, 'default');
    });
}
...
async function saveFact(team, { key, be = 'is', reply = false, value }) {
    ...
    factoids[index] = { key: index, be: be, reply: reply, value: value };
    await util.promisify(controller.storage.teams.save)(factoids);
}
